const { HSB2RGB, RGB2HSB } = require("./colorutils.js");
...
ColorSelector.prototype = {
	...
	init: function (gradient) {
		...
		this.luminosityPosition = [
			gradient.width - 15,
			(gradient.height - 15) / 2,
		];

		this.luminositySelector = document.createElement("canvas");
		this.luminositySelector.style.position = "absolute";
		this.luminositySelector.style.left =
			this.luminosityPosition[0] - 7 + "px";
		this.luminositySelector.style.top =
			this.luminosityPosition[1] - 7 + "px";
		...
		function update(x, y) {
			...
			if (d < 90) {
				...
				scope.luminositySelector.style.left =
					scope.luminosityPosition[0] - 7 + "px";
				scope.luminositySelector.style.top =
					scope.luminosityPosition[1] - 7 + "px";
			}
	...
	setColor: function (color) {
		...
		this.hueSelector.style.left =
			Math.cos(angle) * distance + 125 - 7 + "px";
		this.hueSelector.style.top =
			Math.sin(angle) * distance + 125 - 7 + "px";
	...
	updateLuminosity: function (color) {
		...
		for (i = 0; i < count; i++) {
			...
			context.strokeStyle =
				"rgb(" +
				Math.floor(color[0] - shade) +
				"," +
				Math.floor(color[1] - shade) +
				"," +
